...
const landingPageData_1 = __importDefault(require("./user/routes/landingPageData"));
...
const syncSubtitles_1 = __importDefault(require("./user/controllers/syncSubtitles"));
const getSafeFilePath_1 = __importDefault(require("./user/controllers/getSafeFilePath"));
...
app.get('/video/:filename(*)', (req, res) => {
    ...
    const videoPath = (0, getSafeFilePath_1.default)(VIDEO_PATH, filename);
    if (!videoPath) {
...
app.get('/series/:filename(*)', (req, res) => {
    const { filename } = req.params;
    const videoPath = (0, getSafeFilePath_1.default)(SERIES_PATH, filename);
    if (!videoPath) {
        return res.status(400).send('Invalid file path');
    }
    videoStat(videoPath, req, res);
...
app.use('/user/landing_page_data', authenticateToken_1.authenticateToken, landingPageData_1.default);
...
app.use('/admin/subtitles', syncSubtitles_1.default);
app.listen(PORT, () => {
