      ...
      if(movie.is_series ){
        ...
        if(progress){
          s.seasons.map((season, i) =>{
            ...
            if(findEpisode){
              const { episode_order, video_url, thumbnail_path, subtitles_url, episode_id} = findEpisode;
              const playVideo = { subtitles_url, video_id: movie.video_id, episode_id, progress, completed,
                ...
                episode_order, show_details: true 
              };
          ...
          const { episode_order, video_url, thumbnail_path, subtitles_url, episode_id} = episodes[0];
          const playVideo = { subtitles_url, video_id: movie.video_id, episode_id, progress, completed,
        ...
        const {backdrop_path, description, is_series, video_url, subtitles_url, progress, completed } = m;
        const playVideo = {backdrop_path, description, is_series, video_url, episode_order: 0, 
